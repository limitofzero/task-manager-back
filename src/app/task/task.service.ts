...
export class TaskService {
  ...
  public getAll(): Observable<Task[]> {
    return this.client
      .queryAll<TaskRow>(`${this.BASE_REQ};`)
      .pipe(map((result) => this.transformRows(result)));
  }

  public getTasksByPerformerId(id: string): Observable<Task[]> {
    return this.client
      .queryAll<TaskRow>(`${this.BASE_REQ} WHERE performer_id = $1;`, [id])
      .pipe(map((result) => this.transformRows(result)));
  }

  public getTasksByCreatorId(id: string): Observable<Task[]> {
    return this.client
      .queryAll<TaskRow>(`${this.BASE_REQ} WHERE creator_id = $1;`, [id])
      .pipe(map((result) => this.transformRows(result)));
  }

  public getTasksByProjectId(id: string): Observable<Task[]> {
    return this.client
      .queryAll<TaskRow>(`${this.BASE_REQ} WHERE project_id = $1;`, [id])
      .pipe(map((result) => this.transformRows(result)));
  }
  ...
  public createTask(createTaskDto: CreateTaskDto): Observable<Task> {
    ...
    return this.isUserHasInProject(creatorId, projectId).pipe(
      mergeMap((isCreatorInProject) => {
        return isCreatorInProject ? of(true) : throwError(`Project hasn't user ${creatorId}`);
      }),
      mergeMap(() => {
        return performerId ? this.isUserHasInProject(performerId, projectId) : of(true);
      }),
      mergeMap((isPerformerInProject) => {
        return isPerformerInProject ? of(true) : throwError(`Project hasn't user ${projectId}`);
      }),
      mergeMap(() =>
  ...
  private getTaskById(id: number): Observable<Task> {
    console.log(id);
    return this.client
      .queryOne<TaskRow>(`${this.BASE_REQ} WHERE tasks.id = $1;`, [id])
      .pipe(map((result) => this.transformRow(result)));
  }
  ...
  private isUserHasInProject(userId: string, projectId: string): Observable<boolean> {
    return this.client
      .queryOne(
        `
